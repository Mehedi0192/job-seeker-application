...
class General_category_model extends CI_Model {

	var $table = 'general_category';
	var $column_order = array(null, 'category_name', 'bn_category_name');
	var $column_search = array('category_name', 'bn_category_name');
	var $order = array('sorted_order' => 'ASC');
	...
	public function skill_level () {
		$this->db->select('*');
		$this->db->from('skill_level');
		return $this->db->get()->result();				
	}
	
	public function get_skill_by_id($id) {
		$query = $this->db->get_where('skill_level', array('md5(id)' => $id));
		return $query->row();
	}
