	...
	public function check_user() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (isset($result)) {
				...
				$this->session->set_tempdata("msg", "<span class='success'>Welcome! You are successfully logged.</span>", 5);
				redirect('dashboard');
			} else {
	...
	public function reset_password() {
		...
		if(isset($user)) {
			...
			$msgBody = "<p>Dear ".$user->name.",<br/>"
					. "If this e-mail does not apply to you please ignore it. It appears that you have requested a password reset at our <b>".$contact->company_name."</b> website.<br/>"
					. "To reset your password, please <a href='" . base_url() . 'login/reset_confirm/' . md5($user->email) . "'>click here</a>.</p>"
					. "Thanks,<br/>"
					. "The Administration";
			
			$this->email->from($contact->email, $contact->company_name);
