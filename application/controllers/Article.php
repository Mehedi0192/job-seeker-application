...
class Article extends CI_Controller {
	...
	var $dir = 'admin/article/';
	var $file_path = 'article';
	var $breadcrumb = 'Article';
	...
	public function get_all() {
		$list = $this->article_model->get_all();
		$data = array();
		$i = $_POST['start'];
		foreach ($list as $item) {
			...
			$row = array();
			$row[] = sprintf("%02d", $i);
			$row[] = $item->title . ' <a href="#entry' . $item->id . '" role="button" class="label label-warning" data-toggle="modal">Details</a>' .
			...
			$row[] = $item->bn_title . ' <a href="#entryb' . $item->id . '" role="button" class="label label-warning" data-toggle="modal">বিস্তারিত</a>' .
				'<div id="entryb' . $item->id . '" class="modal fade">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">X</button>
							<h4 id="myModalLabel1"> '.$item->bn_title.' </h4>
						</div>
						<div class="modal-body">
							' . nl2br($item->bn_details) . '
						</div>
					</div>
				</div>
			</div>';
			...
			if($item->file_name) {
				$row[] = "<img src='".base_url().'assets/filemanager/article/'.$item->file_name."' width='50px' class='img-responsive' /> " ;
			} else {
				$row[] = '';
			}
			$action = "<form method='post' action='".base_url()."article/edit' class='actionForm' onClick='return altEdit();' >"
			...
			$action .= "<form method='post' action='".base_url()."article/delete' class='actionForm' onClick='return altDelete();' >"
						."<input type ='hidden' name='id' value='".md5($item->id)."' />"
	...
	public function save() {
		...
		$this->form_validation->set_rules('bn_title', 'bangla title', 'trim|required|xss_clean');
		$this->form_validation->set_rules('bn_details', 'bangla details', 'trim|required|xss_clean');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'bn_details' => $this->input->post('bn_details'),
				'file_name' => $file_name
			);
	...
	public function delete() {
		...
		if ($this->article_model->delete($id) == FALSE) {
			...
			$this->master_model->delete_file($this->file_path, $article->file_name);
